...
;(function(root, factory) {
        ...
        boxCell.find(".jedatebot .jedateok").on("click", function(ev) {
          ...
          if(!opts.onClose){
              ...
              var okymd = ishhmat
                      ...
                      : jet.reMacth(boxCell.children("ul").attr("dateval")),


                  okformat = $($(jet.boxelem).attr(jefix)).length > 0
                      ? boxCell.attr("jeformat")
                      : jet.format,
                  okVal = jet.parse([parseInt(okymd[0]), parseInt(okymd[1]),
                          parseInt(okymd[2])],
                      [okhms[0]||00, okhms[1]||00, okhms[2]||00], okformat),
            ...
            that.dateClose();
            if ($.isFunction(opts.okfun) || opts.okfun != null) opts.okfun(elemCell,elemCell.val(),null);
    ...
    jedfn.chooseDays = function(opts,boxCell) {
        ...
        boxCell.find(".jedaul li").on("click", function(ev) {
            ...
            ev.stopPropagation();
            boxCell.find(".jedaul li.action").removeClass("action");
             _that.attr("class","action");
            //获取时分秒的集合
            boxCell.find(".jedatehms input").each(function() {
                liTms.push($(this).val());
            });
